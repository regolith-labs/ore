...
pub fn process_automate(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    if let Ok(miner) = miner_info.as_account_mut::<MinerV4>(&ore_api::ID) {
        process_automate_v4(accounts, data)
    } else {
        process_automate_v1(accounts, data)
    }
}

/// Sets the executor.
pub fn process_automate_v1(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    // Parse data.
...
/// Sets the executor.
pub fn process_automate_v4(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    // Parse data.
    let args = Automate::try_from_bytes(data)?;
    let amount = u64::from_le_bytes(args.amount);
    let deposit = u64::from_le_bytes(args.deposit);
    ...
    // Open miner account.
    let miner = if miner_info.data_is_empty() {
        create_program_account::<MinerV4>(
            miner_info,
            system_program,
            &signer_info,
            &ore_api::ID,
