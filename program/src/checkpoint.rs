...
pub fn process_checkpoint(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    if let Ok(round) = round_info.as_account::<RoundV4>(&ore_api::ID) {
        process_checkpoint_v4(accounts, data)
    } else {
        process_checkpoint_v1(accounts, data)
    }
}

/// Checkpoints a miner's rewards.
pub fn process_checkpoint_v1(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Load accounts.
    let clock = Clock::get()?;
    let [signer_info, board_info, miner_info, round_info, treasury_info, system_program] = accounts
    else {
    ...
    let required_rent = Rent::get()?.minimum_balance(MinerV1::SIZE);
    assert!(
...
/// Checkpoints a miner's rewards.
pub fn process_checkpoint_v4(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Load accounts.
    let clock = Clock::get()?;
    let [signer_info, board_info, miner_info, round_info, treasury_info, system_program] = accounts
    else {
    ...
    // If round is current round, or the miner round ID does not match the provided round, return.
    let round = round_info.as_account_mut::<RoundV4>(&ore_api::ID)?; // Round has been closed.
    sol_log(&format!("Round ID: {}", round.id).as_str());
    if round.id == board.round_id || round.id != miner.round_id || round.slot_hash == [0; 32] {
        sol_log(&format!("Round not valid").as_str());
        return Ok(());
