...
pub fn process_checkpoint(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    if accounts.len() == 6 {
        return process_checkpoint_v1(accounts, data);
    } else {
        return process_checkpoint_v2(accounts, data);
    }
}

/// Checkpoints a miner's rewards.
fn process_checkpoint_v1(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Load accounts.
    let clock = Clock::get()?;
    ...
    if rewards_sol > 0 {
        match automation {
            Some(automation) if automation.reload > 0 => {
                sol_log(
                    &format!("Sending {} SOL to automation", lamports_to_sol(rewards_sol)).as_str(),
                );
                miner.rewards_sol -= rewards_sol;
