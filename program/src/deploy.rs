...
pub fn process_deploy(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    if accounts.len() > 9 {
        return process_deploy_new(accounts, data);
    } else {
        return process_deploy_old(accounts, data);
    }
}
...
fn process_deploy_new(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    // Parse data.
...
fn process_deploy_old(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    // Parse data.
    let args = Deploy::try_from_bytes(data)?;
    let mut amount = u64::from_le_bytes(args.amount);
    let mask = u32::from_le_bytes(args.squares);
