...
pub fn process_deploy(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    if let Ok(miner) = miner_info.as_account_mut::<MinerV4>(&ore_api::ID) {
        process_deploy_v4(accounts, data)
    } else {
        process_deploy_v1(accounts, data)
    }
}

/// Deploys capital to prospect on a square.
pub fn process_deploy_v1(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    // Parse data.
...
/// Deploys capital to prospect on a square.
pub fn process_deploy_v4(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    // Parse data.
    let args = Deploy::try_from_bytes(data)?;
    let mut amount = u64::from_le_bytes(args.amount);
    let mask = u32::from_le_bytes(args.squares);
