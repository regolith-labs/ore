...
pub fn process_reload_sol(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    ...
    if let Ok(miner) = miner_info.as_account_mut::<MinerV4>(&ore_api::ID) {
        process_reload_sol_v4(accounts, _data)
    } else {
        process_reload_sol_v1(accounts, _data)
    }
}

/// Claims a block reward.
pub fn process_reload_sol_v1(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Load accounts.
    let clock = Clock::get()?;
    let [signer_info, automation_info, miner_info, system_program] = accounts else {
        return Err(ProgramError::NotEnoughAccountKeys);
...
/// Claims a block reward.
pub fn process_reload_sol_v4(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Load accounts.
    let clock = Clock::get()?;
    let [signer_info, automation_info, miner_info, system_program] = accounts else {
        return Err(ProgramError::NotEnoughAccountKeys);
