...
pub fn process_reset(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    ...
    if let Ok(round) = round_info.as_account::<RoundV4>(&ore_api::ID) {
        process_reset_v4(accounts, data)
    } else {
        process_reset_v1(accounts, data)
    }
}

/// Pays out the winners and block reward.
pub fn process_reset_v1(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Load accounts.
    let clock = Clock::get()?;
    let (ore_accounts, other_accounts) = accounts.split_at(14);
...
/// Pays out the winners and block reward.
pub fn process_reset_v4(accounts: &[AccountInfo<'_>], _data: &[u8]) -> ProgramResult {
    // Load accounts.
    let clock = Clock::get()?;
    let (ore_accounts, other_accounts) = accounts.split_at(14);
    sol_log(&format!("Ore accounts: {:?}", ore_accounts.len()).to_string());
    ...
    // Open next round account.
    create_program_account::<RoundV4>(
        round_next_info,
        ore_program,
        signer_info,
        &ore_api::ID,
    ...
    let round_next = round_next_info.as_account_mut::<RoundV4>(&ore_api::ID)?;
    round_next.id = board.round_id + 1;
    round_next.deployed = [0; 25];
    round_next.slot_hash = [0; 32];
    round_next.count = [0; 25];
